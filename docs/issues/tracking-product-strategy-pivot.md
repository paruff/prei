...
>
> Full context: [`docs/PRODUCT_STRATEGY.md`](../docs/PRODUCT_STRATEGY.md)
...
1. **No depreciation or tax modeling** — The single biggest gap for sophisticated passive
   investors. 27.5-year straight-line depreciation, after-tax cash flow, PAL rules, and
   after-tax IRR are all missing. A passive investor's after-tax return is often 30–50% better
   than the pre-tax number; not modeling it undersells every deal. See #NEW-depreciation.
...
2. **No hold-period or exit analysis** — Every passive investor thinks in 5/7/10-year windows.
   ...
   appreciation scenarios, and net sale proceeds are all missing. See #NEW-hold-period.
...
3. **`score_listing_v1` is not an underwriting score** — It uses PPSF + freshness. The 1% Rule,
   ...
   investors use. See #NEW-underwriting-score-v2.
...
4. **No market selection intelligence** — Top .01% investors select markets *before* properties.
   ...
   absent. See #NEW-market-selection.
...
|---|---|---|
| **Phase 1** | Underwriting Engine | #28 (property report), #27 (CMA), + new depreciation, hold-period, score-v2 |
| **Phase 2** | Market Intelligence | #29 (time-series), new market selection, new BRRRR |
| **Phase 3** | Portfolio Tracking | #35 (what-if), #30 (logging) |
| **Phase 4** | Automation & Scale | #31 (ranking), #32 (recommendations), #34 (saved search), #38 (premium data), #19 (gov lists) |
...
- #29 Monthly Trend Time-Series → Confirmed Phase 3
- #30 Structured Logging → Confirmed Phase 4 ops prerequisite
...
- #33 CRM Workflow → Confirmed Phase 5
