...
- [ ] `estimate_arv(comparable_prices: list[Decimal], sqft: Decimal, subject_sqft: Decimal) -> Decimal`
  - Estimates After-Repair Value from comparable sales: `median_ppsf(comps) × subject_sqft`
  - Raises `ValueError` if `comparable_prices` is empty or `sqft <= 0`

- [ ] `estimate_rehab_cost(sqft: Decimal, renovation_level: str) -> Decimal`
  - `renovation_level` must be one of: `"cosmetic"`, `"moderate"`, `"full_gut"`
  - Uses `settings.REHAB_COST_PER_SQFT` dict (keys: cosmetic/moderate/full_gut) for rates
...
- [ ] `brrrr_coc_return(annual_net_cash_flow: Decimal, cash_left_in_deal: Decimal) -> Decimal`
  - If `cash_left_in_deal <= 0`: returns `Decimal("Infinity")` (infinite CoC)
  - Otherwise: `annual_net_cash_flow / cash_left_in_deal`
  - Raises `ValueError` if `annual_net_cash_flow` is zero and `cash_left_in_deal <= 0`

#### `core/services/brrrr.py` (new)

- [ ] `analyze_brrrr_deal(listing_id: int, renovation_level: str, comparable_listing_ids: list[int]) -> dict`
  - Fetches `Listing` and comparables from DB
...
- [ ] `tests/test_brrrr.py`:
  - [ ] `estimate_arv`: empty comps (should raise), single comp, multiple comps
  ...
  - [ ] `brrrr_coc_return`: infinite CoC scenario, normal scenario, zero annual cash flow with positive cash left
