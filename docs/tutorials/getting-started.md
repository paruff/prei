...
If using local dev with `collectstatic`-based storage, build static files:

```bash
python manage.py collectstatic --noinput
```

> **Note:** `staticfiles/` is a build artifact (generated by `collectstatic`) and is gitignored.
> You must regenerate it when static assets change.
...
To get started quickly, load sample property data:
...
This creates a demo superuser and sample properties:
- Email: `demo@prei.dev`
...
## Step 10: View Investment Analysis
