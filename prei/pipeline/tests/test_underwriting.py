...
BASE_INPUT = UnderwritingInput(
    purchase_price=Decimal("300000.0"),
    estimated_rent=Decimal("2500.0"),
    vacancy_rate=Decimal("0.05"),
    rehab_budget=Decimal("20000.0"),
    property_tax_annual=Decimal("3600.0"),
    insurance_annual=Decimal("1200.0"),
    maintenance_reserve_rate=Decimal("0.10"),
    management_fee_rate=Decimal("0.08"),
    hoa_annual=Decimal("600.0"),
)
...
class TestPureArithmetic:
    ...
    def test_net_operating_income(self):
        assert float(net_operating_income(28_500, 10_680)) == pytest.approx(17_820.0)
        assert net_operating_income(0, 0) == 0.0
        assert float(net_operating_income(10_000, 15_000)) == pytest.approx(-5_000.0)

    # ── Division by zero guards ──────────────────────────────────────────────

    def test_cap_rate_zero_price(self):
        assert cap_rate(annual_noi=10_000, purchase_price=0) == 0.0

    def test_cap_rate_negative_price(self):
        # Canonical investor_app.finance.utils.cap_rate has no special case
        # for negative purchase_price - it performs real division.
        assert cap_rate(annual_noi=10_000, purchase_price=-100) == -100.0
    ...
    def test_cash_on_cash_all_cash(self):
        ...
        assert float(coc) == pytest.approx(0.0594, rel=1e-4)
    ...
    def test_cash_on_cash_with_rehab(self):
        ...
        assert float(coc) == pytest.approx(0.0556875, rel=1e-5)
    ...
    def test_max_allowable_offer_standard(self):
        ...
        assert float(mao) == pytest.approx(222_750.0)
...
class TestSolveUnderwriting:
    ...
    def test_base_case(self):
        ...
        assert float(result.noi) == pytest.approx(17_820.0, rel=1e-4)
        assert float(result.cap_rate) == pytest.approx(0.0594, rel=1e-4)
        assert float(result.cash_on_cash) == pytest.approx(0.0556875, rel=1e-5)
        assert float(result.mao) == pytest.approx(222_750.0, rel=1e-4)
    ...
    def test_high_target_cap_rate_lowers_mao(self):
        ...
        assert float(result_10.mao) == pytest.approx(
            178_200.0, rel=1e-4
        )  # 17820 / 0.10
    ...
    def test_low_target_cap_rate_raises_mao(self):
        ...
        assert float(result_6.mao) == pytest.approx(297_000.0, rel=1e-4)  # 17820 / 0.06
    ...
    def test_no_rehab(self):
        ...
        assert float(result.cash_on_cash) == pytest.approx(0.0594, rel=1e-4)
    ...
    def test_higher_vacancy_lowers_noi(self):
        ...
        assert float(result.noi) == pytest.approx(15_060.0, rel=1e-4)
    ...
    def test_all_defaults(self):
        ...
        inp = UnderwritingInput(
            purchase_price=Decimal("200000.0"),
            estimated_rent=Decimal("1800.0"),
            property_tax_annual=Decimal("2400.0"),
            insurance_annual=Decimal("900.0"),
        )
        result = solve_underwriting(inp, target_cap_rate=0.08)
        assert isinstance(result, UnderwritingMetrics)
        assert result.noi > 0
