...
def calculate_property_tax(
    ...
    """
    return (to_decimal(property_value) * to_decimal(tax_rate_percent) / Decimal(100)).quantize(
        Decimal("0.01")
    )
...
def calculate_maintenance_reserve(
    property_value: Decimal, year_built: int = 2000, annual_percent: Decimal = Decimal("1.0")
) -> Decimal:
    ...
    """
    base_maintenance = to_decimal(property_value) * to_decimal(annual_percent) / Decimal(100)
